...
void recognizeAndTrainUsingWebcam(VideoCapture &videoCapture, CascadeClassifier &faceCascade, CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2)
{
    ...
    while (true) {
        ...
        if (faceRect.width > 0) {
            rectangle(displayedFrame, faceRect, CV_RGB(0, 255, 0), 2, CV_AA);
        ...
        if (m_mode == MODE_DETECTION) {
            ...
            if (gotFaceAndEyes) {
                ...
                if ((imageDiff > CHANGE_IN_IMAGE_FOR_COLLECTION) && (timeDiff_seconds > CHANGE_IN_SECONDS_FOR_COLLECTION)) {
					...
					////保存脸部图片到指定目录并定义图片名为编号				
					//Mat faceimg;
					//faceimg = preprocessedFace.clone();

					// //显示图像
					// imshow("image", faceimg);
					// //保存图像
		   //        sprintf(outpath,"E:\\opencvproject\\face_detect_recogn\\face_detect_recogn\\faceimg\\%d.jpg",m_selectedPerson);
		   //        imwrite(outpath,faceimg);
	    ...
	    //             {    
	    //                 	char img_name[100];  
	   //                  	sprintf(img_name,"D://%d.jpg",count);
	   //            	IplImage * result=cvLoadImage(img_name,0);
       //          }
    ...
    //            m_gui_faces_top = BORDER;
               ...
               if (similarity < UNKNOWN_PERSON_THRESHOLD) {
				...
				identity = model->predict(faceimgs);
...
int main(int argc, char *argv[])
{
    ...
    setMouseCallback(windowName, onMouse, 0);
	   ...
	   //             {    
	   //               char inputpath[1000];  
	   //           	sprintf(inputpath,"E:\\opencvproject\\face_detect_recogn\\face_detect_recogn\\faceimg\\%d.jpg",count);
				//	Mat result=imread(inputpath,0);
				//	Mat faceimg;
				//	faceimg = result.clone();
				//	faceimgs.push_back(faceimg);
				//	imshow("hhh",result);
				//}
				//for(vector<Mat>::iterator  iter =faceimgs.begin();iter!=faceimgs.end();iter++)
				//{
				//	imshow("hhh");
				//}
